...
class Network {
public:
	using mvp_t = std::pair<std::vector<Eigen::MatrixXf>, std::vector<Eigen::MatrixXf>>;
	...
	void updateMinibatch(Eigen::Index startBatch, Eigen::Index len, float eta, float lmbda);
	...
	void initDeltas(std::vector<Eigen::MatrixXf>& deltaWs, std::vector<Eigen::MatrixXf>& deltaBs);
public:
	Network(std::initializer_list<Eigen::Index> sizes,
		const std::string& imgPath, const std::string& labelPath,
		int trainingSize, int testSize, int validationSize);
