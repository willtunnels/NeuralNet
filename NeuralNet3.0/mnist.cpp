...
std::array<Eigen::MatrixXf, 3> mnist::read_mnist_images(const std::string& full_path, int trainingSize, int testSize, int validationSize) {
	...
	if (file.is_open()) {
		...
		int actual_nm_imgs;
		file.read((char*)&actual_nm_imgs, sizeof(actual_nm_imgs)), actual_nm_imgs = reverseInt(actual_nm_imgs);

		assert(actual_nm_imgs >= trainingSize + testSize + validationSize); //temporary code for testing purposes
...
std::array<mnist::label_t, 3> mnist::read_mnist_labels(const std::string& full_path, int trainingSize, int testSize, int validationSize) {
	using namespace Eigen;
	std::ifstream file(full_path, std::ios::binary);
	if (file.is_open()) {
		int magic_number = 0;
		...
		file.read((char*)&actual_nm_labels, sizeof(actual_nm_labels)), actual_nm_labels = reverseInt(actual_nm_labels);

		assert(actual_nm_labels >= trainingSize + testSize + validationSize); //temporary code for testing purposes
		...
		std::array<mnist::label_t, 3> _dataset = { Map<mnist::label_t>(buff, trainingSize),
